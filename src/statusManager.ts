...
export class StatusManagerProvider implements vscode.TreeDataProvider<Status> {
  ...
  getChildren(element?: Status): Thenable<Status[]> {
    if (element) {
      ...
      let status: Status[] = [];
      status.push(new Status("port", "", this.port, vscode.TreeItemCollapsibleState.None));
      return Promise.resolve(status);

    } else {
      let status: Status[] = [];
      if (this.installed) {
        let state = (this.running) ? vscode.TreeItemCollapsibleState.Expanded : vscode.TreeItemCollapsibleState.None;
        status.push(new Status("cq_vscode", this.running ? "running" : "stopped", "", state));
      }
      return Promise.resolve(status);
    }
  }
...
export class Status extends vscode.TreeItem {
  constructor(
    ...
    if (running !== "") {
      ...
      this.tooltip = (this.running === "running") ? "CadQuery Viewer is running" : "CadQuery Viewer is stopped";
...
export function createStatusManager() {
  ...
  vscode.window.createTreeView('cadqueryStatus', { treeDataProvider: statusManager });
